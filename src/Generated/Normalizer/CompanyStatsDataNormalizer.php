...
class CompanyStatsDataNormalizer implements DenormalizerAwareInterface, DenormalizerInterface, NormalizerAwareInterface, NormalizerInterface
{
    ...
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        ...
        if (\array_key_exists('invoice_count', $data)) {
            $object->setInvoiceCount($data['invoice_count']);
            unset($data['invoice_count']);
        }
        if (\array_key_exists('last_invoice_at', $data) && $data['last_invoice_at'] !== null) {
            $date = \DateTime::createFromFormat('Y-m-d\TH:i:sP', $data['last_invoice_at']);
            if ($date === false) {
                throw new InvalidDateException($data['last_invoice_at'], 'Y-m-d\TH:i:sP');
            }
            $object->setLastInvoiceAt($date);
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if ($data->isInitialized('lastInvoiceAt') && $data->getLastInvoiceAt() !== null) {
            $dataArray['last_invoice_at'] = $data->getLastInvoiceAt()?->format('Y-m-d\TH:i:sP');
        }
